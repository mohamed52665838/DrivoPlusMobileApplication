...
const LoginScreen = () => {
  ...
  const { handleSubmit, control, formState: { errors, isSubmitting } } = useForm<LoginFormData>();
...
const onLogin = async (data: LoginFormData) => {
  const response = await signin(data).catch((e: AxiosError) => {
    const [title, message] = (e.response?.status === 400)
  ...
  if (response?.data?.user) {
    await Promise.all([
      ...
      storeApiKey(response.data.tokens.refresh_token, TokenStructure.REFRESH_TOKEN)
    ]);
...
>
  LOG IN
        ...
        <View style={styles.footerText}>
          ...
          <Button onPress={() => router.push('/signupscreen')} textColor="#4CAF50">
            Membership
          </Button>
        </View>
      </View>
